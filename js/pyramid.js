...
var $KakusaiData={};  //グローバル変数 {100:{male:xx,female:xx},99:{male:xx,female:xx}・・・
var $nengetsu = 0;
var $hitoku = false;
...
//alert(pyramidData["shiku"]);
  ...
  function source_str(shiku,source){
    ...
    if ( shiku=="横浜市" && nengetsu.match(/年/) ){
      ...
      var stat  = "横浜市将来推計人口";
      var url   = "https://www.city.yokohama.lg.jp/city-info/seisaku/torikumi/shien/jinkosuikei.html";
...
function get_SendData(mode,nengetsu){
  ...
  if(nengetsu==undefined){nengetsu=get_selected_nengetsu();}
  if(nengetsu==undefined){nengetsu=$nengetsu}
...
function get_source_url(nengetsu){
  ...
  if     ( yy<=19 ){ var gen = "h"+(yy+12) }
  else if( yy<=59 ){ var gen = "r"+(yy-18) }
  else if( yy<=88 ){ var gen = "s"+(yy-25) }
  else if( yy<=99 ){ var gen = "h"+(yy-88) }

  var shiku   =get_selected_shiku();
  var pyramode=get_pyramid_mode();
  if( yy<3 || 74<yy ){
    var Location1 = "https://www.city.yokohama.lg.jp/city-info/yokohamashi/tokei-chosa/portal/jinko/choki.files/4.xlsx"
  }else{
    var Location1 = "https://www.city.yokohama.lg.jp/city-info/yokohamashi/tokei-chosa/portal/jinko/nenrei/suikei.files/<yy>_hyo22.xlsx";
  }
  
  var Location2 = "https://www.city.yokohama.lg.jp/city-info/yokohamashi/tokei-chosa/portal/jinko/chocho/nenrei/<gen>cho-nen.files/<ku><nengetsu>.csv";
  switch(pyramode){
    case "shiku":
      var src = Location1;
      break;
  ...
  src = src.replace(/<nengetsu>/g,nengetsu).replace(/<ku>/g,shiku).replace(/<yy>/g,yy).replace(/<gen>/g,gen);
  return src;
}
...
function set_another_nengetsu(newPyramode){
  ...
  if(newPyramode=="shiku"){
    ...
    if(nengetsu.match(/年/)){
      ...
      var pre_ku_nen = ku.options[get_option_id(ku,"new")+1].value.substr(0,4);      
...
function change_seireki_main(hi){
  ...
  if     (gen=="大正"){var seireki=1911+num_nen}
  else if(gen=="昭和"){var seireki=1925+num_nen}
  else if(gen=="平成"){var seireki=1988+num_nen}
  else if(gen=="令和"){var seireki=2018+num_nen}    
...
function screen_shot(){
  ...
  //--年齢3区分のラインがhtml2canvasでは描画されないので,trのborder-bottomをnoneにする。
  var trs=document.getElementsByClassName("line");
  for(var i=0;i<4;i++){
    trs[i].setAttribute("style","none");
  }
  //--年齢3区分のラインの代替措置としてbarのdivにborder-bottomを設定する。
  var bars=["mb0","fb0","mb15","fb15","mb65","fb65","mb75","fb75"];
  var w_obj = {};
  for(var i=0;i<bars.length;i++){
    let bar = document.getElementById(bars[i]);
    w_obj[bars[i]] = bar.getAttribute("style");
    bar.setAttribute("style",w_obj[bars[i]]+"height:5px;border-bottom:1px solid #ee82ee;")
  }
  ...
  mlabel.setAttribute("style","border:none;left:200px")
  flabel.setAttribute("style","border:none;right:200px")
  ...
  var y_pos   = rect1.top;
  if(h2.clientWidth < table.clientWidth){
    var c_width       = table.clientWidth;
  }else{
    var c_width       = h2.clientWidth;
  }
  var c_height       = (rect2.top-rect1.top)+table.clientHeight;
  window.scrollTo(0,0);
...
　    let bar = document.getElementById(bars[i]);
　    bar.setAttribute("style",w_obj[bars[i]]+"height:6px;")
　  }
    title.innerHTML            = org_txt;
    document.getElementById("male_label").setAttribute("style","border:1px solid blue")
    document.getElementById("female_label").setAttribute("style","border:1px solid red")
  });
}


//スクリーンショットを撮る.
function screen_shot_old(){
  //グラフとグラフタイトルの要素
  var table   = document.getElementById("table");
  var h2      = document.getElementById("h2");
